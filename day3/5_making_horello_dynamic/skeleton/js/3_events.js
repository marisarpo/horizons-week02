...
horello.mountStatic = function() { // this only happens once and does the same thing every time
  ...
  // field).
  $('#addList').on('shown.bs.collapse', function (e) { // bootstrap sends an event every time one of the collapse events is fired 
                                                        // jquery installs new events to the page 
    $('#addListText').focus();
  });
  ...
  $('#addListSave').click(function(e) { // get the id from the inspect element
    var listName = $('#addListText').val();
    // validate input
    if (!$('#addListText').val()) {       // if there is no value, alert (validate this)
      alert("Please enter a list name");
      return;
    }
    board.addList(listName); 
    $('#addListText').val('');           //resets the input field to be empty 
    $('#addList').collapse('hide');     // collapses the toggle 
    horello.mount(board);                  // 
  });

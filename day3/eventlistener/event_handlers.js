...
handlers.attachHoverClick = function(id) {
  handlers.attachHover($("#" + id), function(event){
    var timeout = setTimeout(function(){
        $("#" + id).trigger("click");
    }, 2000);
    ...
    handlers.attachUnhover($('#' + id), function(event){
...
handlers.attachAlertsToClass = function(className, alertMessage) {
  var newArray = $("." + className).toArray(); 
    handlers.attachClick(newArray, function(e){
        alert(alertMessage);
    e.stopPropagation();
    });
...
if (elements.get(0) == document){return; }
    ...
    return handlers.attachAlertsWithParents(elements.parent());
}
};
...
handlers.detachAlertsWithParents = function(elements) {
  if (elements.get(0) == document){
      return;
        ...
        elements.off('click');
        return handlers.detachAlertsWithParents(elements.parent());
    }
};

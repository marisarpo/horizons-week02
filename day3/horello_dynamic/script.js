...
$('.add-card-save').on('click', function() {
    var title = $('.add-list-form-wrapper input').val();
    var x = this.closest('.list-footer').sibling('.list-cards')
    $(x).append('<div class="card">');
        $(x.children('card:last')).append('<span class="card-more">');
            $(x.children('card:last').children('.card-more'));
        $(x.children('card:last')).append('<div class="card-body">' + title + '</div>');

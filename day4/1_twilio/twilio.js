...
// We're going to build an application that lets you `shoutout` (one-way message) a Twilio-enabled phone number programmatically.
// You'll be able to send yourself a text message from your browser (and much more)!
...
// You're going to be signing up for the service yourself, and it won't be asking you to pay for anything.
// Enter your name, phone number, and whatever else they ask you for (within reason - if they're asking for an ssn, call one of us over)
...
twilio.accountId = "ACe33b8c58cde962e2071c9777575b952a";
twilio.authToken = "9df7494376425b893a93ebcaf202428b";
twilio.fromNumber = "16176525596 ";

// Exercise 1. Implement the `initialize` method
// When the TwilioShoutout class is constructed, it calls its initialize() method.
// That method should set up event listener(s) that will allow you to capture and send data from your web UI.
...
// The function takes a string and returns true or false based on whether the string is a valid message or not.
// More information is provided at the method.
...
// The function takes a string and returns true or false based on whether the string is a valid phone number or not.
// Validating phone numbers can be extremely hard, so we're going to be using the simple no-parentheses,
// non-hyphenated phone number format, including international code.
// I.E. +1 (201) - 705 - 1234 becomes 12017051234. More information is provided at the method.
...
// Otherwise, it should raise an alert, telling the user (you) that one or both of the input fields are invalid.
// This is a multipart exercise that requires a few key steps, and we've provided you with a bit of what you need to get you going.
...
twilio.TwilioShoutout.prototype = {
  ...
  validatePhoneField: function(phoneStr) {
    // YOUR CODE HERE
    return phoneStr.trim().length ? !isNaN(phoneStr) : false;

  },
	// Exercise 5. `handleMessageSend(evt<Event>)` method
	// Write a method that will check the validity of the phone and message fields,
  // and if they're both valid, calls the Twilio API with our data so that it can send a text to your phone.
  // If not, it should throw an error "Invalid fields";
	// 
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    // This callback should create a new Message object and generate a JQuery object using its render() method.
    // It should append the gnerated JQuery object to the DOM messageList.
    var cb = function(data) {
			// YOUR CODE HERE
      var msg = new Message(toNumber,messageBody);
      messageList.append(msg.render());
    };

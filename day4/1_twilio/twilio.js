...
twilio.accountId = "ACa865dfa74a8a02d4472c413bd4a47b50";
twilio.authToken = "446447111ac146af8565d53b527f758f540";
twilio.fromNumber = "9789430777";

// Exercise 1. Implement the `initialize` method
// When the TwilioShoutout class is constructed, it calls its initialize() method. That method should set up event listener(s) that will allow you to capture and send data from your web UI.
...
// TwilioShoutout has a `validateMessageField` method that needs to be implemented. The function takes a string and returns true or false based on whether the string is a valid message or not.
// More information is provided at the method.

// Exercise 4. Implement the `validatePhoneField` method
// TwilioShoutout has a `validatePhoneField` method that needs to be implemented. The function takes a string and returns true or false based on whether the string is a valid phone number or not.
// Validating phone numbers can be extremely hard, so we're going to be using the simple no-parentheses, non-hyphenated phone number format, including international code. I.E. +1 (201) - 705 - 1234
// becomes 12017051234. More information is provided at the method.

// Exercise 5. Implement the `handleMessageSend` method
// The `handleMessageSend` method is going to act as the callback for the event listener you set up in the initialize() method. It's supposed to make sure all the necessary fields are valid,
// and if so, make an ajax call to a Twilio API url with our data. Otherwise, it should raise an alert, telling the user (you) that one or both of the input fields are invalid. This is a multipart
// exercise that requires a few key steps, and we've provided you with a bit of what you need to get you going.
...
twilio.TwilioShoutout.prototype = {
  ...
  initialize: function() {
    // YOUR CODE HERE
    // var listeners =  {};
    // $('.messageSendButton').on("click", handleMessageSend());

    // ans - q what's with this??
  ...
  validatePhoneField: function(phoneStr) {
	...
	// Write a method that will check the validity of the phone and message fields, and if they're both valid, calls the
  // Twilio API with our data so that it can send a text to your phone. If not, it should throw an error "Invalid fields";
  ...
  handleMessageSend: function(evt) {
    ...
    // something is invalid, throw error
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    // This callback should create a new Message object and generate a JQuery
    // object using its render() method. It should append the gnerated JQuery
    // object to the DOM messageList.

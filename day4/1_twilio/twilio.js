...
twilio.accountId = 'AC3a13d1ff6cc8d37ddb6d6967aa0a4f35';
twilio.authToken = '33e26e6c2966c4aa68718919e124c470';
twilio.fromNumber = '12532631339';
...
twilio.TwilioShoutout.prototype = {
  ...
  // Implement the initialize method. The initialize method is called when
  // the TwilioApp class is instantiated.
  // It should set up a click handler for `messageSendButton`, and fire `this.handleMessageSend` .
  // hint. remember about context and maybe .bind()? you should, you'll
  // run into some problems if you don't use the right context.
  initialize: function() {
    var self = this;
  ...
  // and if they're both valid, calls the Twilio API with our data so that it can s
//  end a text to your phone. If not, it should throw an error "Invalid fields";
	//
  ...
  handleMessageSend: function(evt) {
    ...
    if (!(this.validatePhoneField(phoneNum)) ||
      ...
      this.sendMessage(phoneNum, messageField);
    }
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    var cb = function(data) {
			var myMessage = new Message(toNumber, messageBody);
      $('.message-list').append(myMessage.render());
    ...
    $.ajax({
      method: "POST",
			...
			// Please examine the API docs for sending messages with
      ...
      url: "https://api.twilio.com/2010-04-01/Accounts/" + this.accountId + "/Messages",
			...
			//
			// note. see the Twilio docs (https://www.twilio.com/docs/api/rest/sending-messages) for
      // more details about these fields you're sending.
      data : {
...
// Nice, you got to the end. Right now, the test is instantiating the app
// and allowing you to run it, but if you wanted to use it yourself
// (removing the tests) you can use it by
// var app = new twilio.TwilioShoutout(twilio.accountId, twilio.authToken,
// twilio.fromNumber)
// Just instantiating the app will set up the event handlers and make the
// UI interactive (as you should know, you built it haha)

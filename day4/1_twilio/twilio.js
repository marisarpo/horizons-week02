...
twilio.accountId = "AC14c02f58d3fa4993ea09ae6930dc6725";
twilio.authToken = "96ea114068ba12e6398a696baec8ba73";
twilio.fromNumber = +19073122529;

// Exercise 1. Implement the `initialize` method
// When the TwilioShoutout class is constructed, it calls its initialize() method. That method 
// should set up event listener(s) that will allow you to capture and send data from your web UI. 
// More information is provided at the method.

// Exercise 2. Implement the `clear` method
// There are going to be two input fields in the application you're building, and it would be 
//nice to have some function to clear their values.
...
//The function takes a string and returns true or false based on whether the string is a 
//valid message or not. More information is provided at the method.

// Exercise 4. Implement the `validatePhoneField` method
// TwilioShoutout has a `validatePhoneField` method that needs to be implemented. The 
//function takes a string and returns true or false based on whether the string is a valid 
//phone number or not. Validating phone numbers can be extremely hard, so we're going to be 
//using the simple no-parentheses, non-hyphenated phone number format, including international 
//code. I.E. +1 (201) - 705 - 1234 becomes 12017051234. More information is provided at the method.

// Exercise 5. Implement the `handleMessageSend` method
// The `handleMessageSend` method is going to act as the callback for the event listener you 
//set up in the initialize() method. It's supposed to make sure all the necessary fields are 
//valid, and if so, make an ajax call to a Twilio API url with our data. Otherwise, it should 
//raise an alert, telling the user (you) that one or both of the input fields are invalid. This 
//is a multipart exercise that requires a few key steps, and we've provided you with a bit of what
...
twilio.TwilioShoutout.prototype = {
  ...
  // Implement the initialize method. The initialize method is called when the TwilioApp class 
  // is instantiated.
  // It should set up a click handler for `messageSendButton`, and fire `this.handleMessageSend` .
	// 
  // hint. remember about context and maybe .bind()? you should, you'll run into some problems 
  // if you don't use the right context.
  initialize: function() {
    this.messageSendButton.on("click", this.handleMessageSend.bind(this));
  },
  // Exercise 2. `clearField(jqField<JQuery Element>)` method
  // Write a function that takes a JQuery input fields and clears the text inside it. It should 
  // not return anything.
  //
  ...
  clearField: function(jqField) {
    // YOUR CODE HERE
    // var trimmed = $.trim(jqField);
    // $(trimmed).val("");
    jqField.val("");
  },
  // Exercise 3. `validateMessageField(textStr<String>)` method
  // Write a function that validates the message input field. It should return true if the 
  // `validateMessageField` passes these conditions: 
  ...
  // Write a function that validates the message input field. It should return true if the 
  // `validatePhoneField` passes these conditions: 
  ...
  //
	...
	// hint. .charAt might be useful, see 
  // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/charAt
  validatePhoneField: function(phoneStr) {
    var number = $.trim(phoneStr);
    if (number.length !== 0) {
      for (var i = 0; i < number.length; i++) {
        if (isNaN(phoneStr.charAt(i))) {
          return false;
        }
      } return true;
    } else { return false }
	...
	// Write a method that will check the validity of the phone and message fields, and if 
  // they're both valid, calls the Twilio API with our data so that it can send a text to 
  // your phone. If not, it should throw an error "Invalid fields";
	// 
  ...
  handleMessageSend: function(evt) {
    ...
    console.log("message handleMessageSend")

    var phoneStr = this.phoneInputField.val();
    var textStr = this.messageInputField.val();

    if (this.validatePhoneField(phoneStr) && this.validateMessageField(textStr)){
      this.sendMessage(phoneStr, textStr);
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    // This callback should create a new Message object and generate a JQuery object using 
    // its render() method. It should append the gnerated JQuery object to the DOM messageList.
    var cb = function(data) {
      //var Message = new Message(twilio.fromNumber, messageBody)
      //messageList is already a jquery object, can only append to a jquery object
			messageList.append(new Message(twilio.fromNumber,messageBody).render());
    };
...
// Nice, you got to the end. Right now, the test is instantiating the app and allowing you to 
// run it, but if you wanted to use it yourself (removing the tests) you can use it by
var app = new twilio.TwilioShoutout(twilio.accountId, twilio.authToken, twilio.fromNumber)
// Just instantiating the app will set up the event handlers and make the UI interactive (as 
  // you should know, you built it haha)

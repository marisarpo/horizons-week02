...
// Preface. In this exercise, you're going to be building a Twilio Shoutout Application that communicates with Twilio's API (Application Programming Inerface) over AJAX. 
// You can think of a Web API as an application black-box that you can leverage and interact with using HTTP. If you wanted to get weather data for your city, 
// you could query some weather API and access the data that application generates.
// In this case, we're going to be using Twilio's device SMS messaging service. We're going to build an application that lets you 
//`shoutout` (one-way message) a Twilio-enabled phone number programmatically. You'll be able to send yourself a text message from your browser (and much more)!
...
// You're going to be signing up for the service yourself, and it won't be asking you to pay for anything. Enter your name, phone number, 
// and whatever else they ask you for (within reason - if they're asking for an ssn, call one of us over)
...
twilio.accountId = "ACb6c2c8f4cd7642fe7935a71fd308aebe";
twilio.authToken = "455710445cee2f7a1e71568c250b0eaa";
twilio.fromNumber = "18053077341";

// Exercise 1. Implement the `initialize` method
// When the TwilioShoutout class is constructed, it calls its initialize() method. That method should set up event listener(s) that will allow you to 
// capture and send data from your web UI. More information is provided at the method.
...
// TwilioShoutout has a `validateMessageField` method that needs to be implemented. The function takes a string and returns true or false based on whether 
// the string is a valid message or not. More information is provided at the method.

// Exercise 4. Implement the `validatePhoneField` method
// TwilioShoutout has a `validatePhoneField` method that needs to be implemented. The function takes a string and returns true or false based on whether 
// the string is a valid phone number or not. Validating phone numbers can be extremely hard, so we're going to be using the simple no-parentheses, 
// non-hyphenated phone number format, including international code. I.E. +1 (201) - 705 - 1234 becomes 12017051234. More information is provided at the method.

// Exercise 5. Implement the `handleMessageSend` method
// The `handleMessageSend` method is going to act as the callback for the event listener you set up in the initialize() method. 
// It's supposed to make sure all the necessary fields are valid, and if so, make an ajax call to a Twilio API url with our data. 
// Otherwise, it should raise an alert, telling the user (you) that one or both of the input fields are invalid. This is a multipart exercise that requires a few key steps, 
...
twilio.TwilioShoutout.prototype = {
  ...
  validatePhoneField: function(phoneStr) {
	...
	// Write a method that will check the validity of the phone and message fields, and if they're both valid, calls the Twilio API 
  // with our data so that it can send a text to your phone. If not, it should throw an error "Invalid fields";
	// 
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    var cb = function(data) {
			var Created = new Message(toNumber, messageBody);
      var Obj = Created.render();

...
// so we're going to be using the simple no-parentheses, non-hyphenated phone number 
// format, including international code. I.E. +1 (201) - 705 - 1234 becomes 12017051234.
...
// the necessary fields are valid, and if so, make an ajax call to a Twilio API url with 
// our data. Otherwise, it should raise an alert, telling the user (you) that one or 
// both of the input fields are invalid. This is a multipart exercise that requires a 
//few key steps, and we've provided you with a bit of what you need to get you going.
...
twilio.TwilioShoutout.prototype = {
  ...
  // Write a function that validates the message input field. It should return 
  //true if the `validatePhoneField` passes these conditions: 
  // (1) The field should not have any non-numeric characters ('201-123-4321' 
  // is bad)
  ...
  validatePhoneField: function(phoneStr) {
    var trimmedNum = $.trim(phoneStr);
    console.log("TESTING", trimmedNum.length)
    if (trimmedNum.length !== 0) {
      for (var i = 0; i< trimmedNum.length; i++) {
        console.log(isNaN(trimmedNum[i]));
        if (isNaN(trimmedNum[i])) {
          return false;
        }
      } return true;
    }
  ...
  handleMessageSend: function(evt) {
    ...
    console.log(messageStr, phoneStr);
    console.log(this.validatePhoneField(phoneStr))



    if (this.validateMessageField(messageStr) &&
      this.validatePhoneField(phoneStr)) {
        this.sendMessage(phoneStr, messageStr);
        return;
      }
    else {
  ...
  // Write a function that POSTS to the Twilio Messages REST Api with a 
  // destination number `toNumber` and message `messageBody`.
  //
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    // object using its render() method. It should append the generated JQuery object 
    // to the DOM messageList.
    var cb = function(data) {
		  var newMessage = new Message(toNumber, messageBody);
      var toDOM = newMessage.render();
      messageList.append(toDOM);
    };
...
// It returns a jQuery object that encloses span and p tags that encapsulate 
//the sender and body properties, respectively.
Message.prototype = {
  render: function() {

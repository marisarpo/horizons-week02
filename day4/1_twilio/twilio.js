...
twilio.accountId = "ACacbe534d58fb70d0138df3add30dbcb6";
twilio.authToken = "25b29f9fdc757fa20dd85cf600f98df5";
twilio.fromNumber = "18329240377";
...
// There are going to be two input fields in the application you're building, and it would 
//be nice to have some function to clear their values.
...
//he function takes a string and returns true or false based on whether the string is a 
...
//a valid phone number or not. Validating phone numbers can be extremely hard, so we're 
//going to be using the simple no-parentheses, non-hyphenated phone number format, 
//including international code. I.E. +1 (201) - 705 - 1234 becomes 12017051234.
...
twilio.TwilioShoutout.prototype = {
  ...
  // Implement the initialize method. The initialize method is called when the TwilioApp 
  ...
  // and if they're both valid, calls the Twilio API with our data so that it can send a 
  // text to your phone. If not, it should throw an error "Invalid fields";
	// 
  ...
  handleMessageSend: function(evt) {
    ...
    var messageText = this.messageInputField.val();
    if (!this.validatePhoneField(toPhone) || !this.validateMessageField(messageText)) {
      throw 'Invalid fields!';
    }
    this.sendMessage(toPhone, messageText);
  ...
  sendMessage: function(toNumber, messageBody) {
    ...
    $.ajax({
      ...
      url: this.apiUrl + "/Accounts/" + acctId + "/Messages",
			// Exercise 6.C `data`
...
// Nice, you got to the end. Right now, the test is instantiating the app and allowing you to run it, 
// but if you wanted to use it yourself (removing the tests) you can use it by

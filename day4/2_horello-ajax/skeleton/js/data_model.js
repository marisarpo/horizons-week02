...
horello.Card.prototype = {
  ...
  setTitle: function(titleStr) {
    this.title = titleStr;
    $.ajax({
      url: "https://api.trello.com/1/cards/" + this.id,
      method: "PUT",
      data: {
        key: horello.apiKey,
...
horello.Board = function () {
  ...
  $.ajax("https://api.trello.com/1/boards/" + this.id + "/lists", {
      method:"GET",
      data:{
        key: horello.apiKey,
        token: horello.apiToken
...
horello.Board.prototype = { // trello board id
  ...
  //method inside new board that loads trello data upon initialization
  //initialize - grab trello data in ajax request -> store in this.lists
  //    ---> this.lists does same for its cards, cards does same for its descriptions
  getList: function(listId) { //make ajax
